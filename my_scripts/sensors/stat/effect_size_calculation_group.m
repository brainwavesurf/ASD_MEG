...
freq_diff_isi_NT = cell(1,size(SUBJ_NT,1));
for s=1:size(SUBJ_NT,1)
    ...
    freq_diff_isi_NT{s} = fft_fast_isi;
    freq_diff_isi_NT{s}.powspctrm = (fft_fast_isi.powspctrm - fft_slow_isi.powspctrm)./(fft_fast_isi.powspctrm + fft_slow_isi.powspctrm);
end

%% load freqanalysis data for asd group
freq_fast_isi_ASD = cell(1,size(SUBJ_ASD,1));
freq_slow_isi_ASD = cell(1,size(SUBJ_ASD,1));
freq_diff_isi_ASD = cell(1,size(SUBJ_ASD,1));
for s=1:size(SUBJ_ASD,1)
    ...
    freq_diff_isi_ASD{s} = fft_fast_isi;
    freq_diff_isi_ASD{s}.powspctrm = (fft_fast_isi.powspctrm - fft_slow_isi.powspctrm)./(fft_fast_isi.powspctrm + fft_slow_isi.powspctrm);
end

cfg = [];
cfg.keepindividual = 'yes';
grandavg_fast = ft_freqgrandaverage(cfg, freq_diff_isi_NT{:});
grandavg_slow = ft_freqgrandaverage(cfg, freq_diff_isi_ASD{:});
...
inference = load(strcat(savepath, '1_results/sensor_clusters/', 'cluster_based_between_groups_stat_grad.mat'));
x1 = nan(45,1);
x2 = nan(45,1);
