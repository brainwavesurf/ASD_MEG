...
for s=1: size (SUBJ,1)
    ...
    freqFast_grad  = freq_all.conv_fast_grad;
    freqSlow_gras  = freq_all.conv_slow_grad;
    %select posterior sensors
    cfg = [];
    cfg.channel = post_sens;
    freqFast_postsens{s}  = ft_selectdata(cfg, freqFast_grad);
    freqSlow_postsens{s}  = ft_selectdata(cfg, freqFast_grad);
...
figure(1)
subplot(2,1,1)
plot(slow_avg_grad.freq, slow_avg_grad.powspctrm(???), '-g'); title('FFT power, avg posterior sens for all subj')
hold on
plot(fast_avg_grad.freq, fast_avg_grad.powspctrm(???), '-b'); 
legend('slow', 'fast'); xlim([5 30]);

subplot(2,1,2)
plot(slow_avg_grad.freq, log(slow_avg_grad.powspctrm(???)), '-g'); title('FFT log power, avg posterior sens for all subj')
hold on
plot(fast_avg_grad.freq, log(fast_avg_grad.powspctrm(???)), '-b'); 
legend('slow', 'fast'); xlim([5 30]);
