...
%     
    ...
    medium_epochs   = ft_selectdata(cfg, epochs); %extraxt trials after fast stimuli
    
    cfg.trials    = fast_ind;
    fast_epochs   = ft_selectdata(cfg, epochs); %extraxt trials after fast stimuli
    
    
    filename = strcat(savepath, subj, '/', subj, '_preproc_epochs_2_40.mat');
    save(filename, 'epochs', 'slow_epochs', 'medium_epochs', 'fast_epochs');
